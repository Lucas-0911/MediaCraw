...
NOTE_ID = "5123456789"
COMMENT_ID = "998877"
# Valid RFC2822 timestamp (weekday aligned with 2025-06-14 Saturday)
RFC2822_TIME = "Sat Jun 14 12:00:00 +0800 2025"

# Forbidden field names. Nickname keys may remain if values are masked.
FORBIDDEN_KEYS = {
...
def make_mock_note() -> dict:
    """Mock note_item shaped like m.weibo.cn, including nested user info."""
    return {
        "mblog": {
            "id": NOTE_ID,
...
def make_mock_comment() -> dict:
    """Mock comment_item shaped like Weibo comments, including nested user info."""
    return {
        "id": COMMENT_ID,
...
class _FakeStore:
    """Capture store_content/store_comment dicts without writing storage."""
...
def _patch_factory(fake: "_FakeStore"):
    """Replace WeibostoreFactory.create_store with a fake staticmethod,
    returning (module, orig) so finally can restore it."""
...
def test_weibo_note_masks_user_info():
    """Flattened note dict has no forbidden keys, hashed creator id, and a masked nickname."""
    import store.weibo as wb
    ...
    # 2. creator_hash is 16-char hex and not the raw user id
    ...
    # 3. Nickname is masked: not the original and contains '*'
    ...
    # 4. Body is stored in content, not desc
...
def test_weibo_comment_masks_user_info():
    """Flattened comment dict has no forbidden keys, hashed creator id, and a masked nickname."""
    import store.weibo as wb
    ...
    # 2. creator_hash exists and is not the raw user id
    creator_hash = captured.get("creator_hash")
...
def test_weibo_store_end_to_end_sqlite():
    """End-to-end: capture note/comment dicts and write/read them through in-memory SQLite ORM.
    ...
    # ---- 1. FakeStore captures dicts from update_weibo_note / update_weibo_note_comment ----
    fake = _FakeStore()
    wb_, orig = _patch_factory(fake)
    try:
        asyncio.run(wb.update_weibo_note(make_mock_note()))
    ...
    # ---- 2. In-memory SQLite, create the two Weibo tables ----
    engine = create_engine("sqlite:///:memory:")
    Base.metadata.create_all(engine, tables=[WeiboNote.__table__, WeiboNoteComment.__table__])
    Session = sessionmaker(bind=engine)
    session = Session()
    try:
        # ---- 3. note: build the ORM object (extra removed columns TypeError) and insert ----
        ...
        # Schema has no forbidden columns
        assert not (note_cols & FORBIDDEN_KEYS), \
            f"WeiboNote 表仍含禁用列: {note_cols & FORBIDDEN_KEYS}"
        # Row data: creator_hash is correct, nickname masked, body kept
        ...
        # Confirm there is no desc column storing a user bio
        assert "desc" not in note_cols

        # ---- 4. comment: same. IDs are strings; create_time stays int ----
