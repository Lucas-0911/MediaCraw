...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        platform: Annotated[
            ...
            typer.Option(
                "--platform",
                help="Media platform selection (xhs=XiaoHongShu | dy=Douyin | ks=Kuaishou | bili=Bilibili | wb=Weibo | tieba=Baidu Tieba | zhihu=Zhihu | tiktok=TikTok VN)",
        ...
        if specified_id_list:
            if platform == PlatformEnum.XHS:
                ...
                config.TIKTOK_SPECIFIED_ID_LIST = specified_id_list
